window.onload = function () {
  ...
  class DropChip {
    ...
    // function to animate droping chip
    animateDropChip(i, color, box, colNum){
      let timing = 100
      let nowColor = color

      for (let j = 5; j >= i; j--) {
        let topBox = document.getElementById(`${j}-${colNum}`)
        if (box !== topBox) {
          setTimeout(function () {
            $(topBox).addClass(`${nowColor}-drop`)
          }, timing)
          setTimeout(function () {
            $(topBox).removeClass(`${nowColor}-drop`)
          }, 1200)
        }
        timing += 200
      }
  ...
  class GameBoard {
    ...
    dropToBottom(clickedElement) {
      ...
      // variable for timing of when dropChip function goes
      let dropInitialTiming = 1100
      ...
      for (let i = 0; i <= 5; i++) {
        ...
        if ($(box).hasClass('red') || $(box).hasClass('black')) {
          ...
          this.chip.animateDropChip(i, this.color, box, colNum)
          // calculates drop timing to be less at higher rows
          this.dropTiming = dropInitialTiming - (i * 200)
          setTimeout(() => { this.chip.dropChip(box) }, this.dropTiming)
          this.turnCount += 1
          return box
          break;
        }
    ...
    vertWinner(rowNum, colNum, rowNumInt, colNumInt) {
      ...
      for (let i = 1; i < 4; i++) {
        ...
        if ($(boxUp).hasClass(this.color)) {
          counter++
        }
        if ($(boxDown).hasClass(this.color)) {
          counter++
        }
  ...
  class App {
    ...
    isPlayable(clickedElement) {
      if (this.board.playable) {
        ...
        // added to adjust timing of win message to match animations
        let winTiming = this.board.dropTiming + 250

        if (this.board.winner(box)) {
          setTimeout(() => {
            alert(`${this.board.color.toUpperCase()} WINNNNS!!!!`)
          }, winTiming)

...
function checkWinner() {
  var board = {};
  var winner = false;
  var WINNERS = [[0,1,2], [3,4,5], [6,7,8], [2,4,6], [1,4,7], [0,3,6], [2,5,8], [0,4,8]];
...
function saveGame() {
  ...
  if (currentGame) {
    ...
    $.post('/games', gameData, function(game) {
      ...
      $("#gameid-" + game.data.id).on('click', function() { reloadGame(game.data.id) });
    });
  }
}
...
function showPreviousGames() {
  ...
  $.get('/games', function(savedGames) {
    if (savedGames.data.length) {
      savedGames.data.forEach(buttonGame);
    }
  });
}

function buttonGame(game) {
  // debugger

  $('#games').append(`<button id="gameid-${game.id}">${game.id}</button> - <em>Updated at: ${game["attributes"]["updated-at"]}</em><br>`);

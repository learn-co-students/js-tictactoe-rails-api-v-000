...
function checkWinner() {
  s1 = { selector: '[data-x="0"][data-y="0"]' };
  s1text = $( Object.values(s1)[0] ).text();
  s2 = { selector: '[data-x="1"][data-y="0"]' };
  s2text = $( Object.values(s2)[0] ).text();
  s3 = { selector: '[data-x="2"][data-y="0"]' };
  s3text = $( Object.values(s3)[0] ).text();
  s4 = { selector: '[data-x="0"][data-y="1"]' };
  s4text = $( Object.values(s4)[0] ).text();
  s5 = { selector: '[data-x="1"][data-y="1"]' };
  s5text = $( Object.values(s5)[0] ).text();
  s6 = { selector: '[data-x="2"][data-y="1"]' };
  s6text = $( Object.values(s6)[0] ).text();
  s7 = { selector: '[data-x="0"][data-y="2"]' };
  s7text = $( Object.values(s7)[0] ).text();
  s8 = { selector: '[data-x="1"][data-y="2"]' };
  s8text = $( Object.values(s8)[0] ).text();
  s9 = { selector: '[data-x="2"][data-y="2"]' };
  s9text = $( Object.values(s9)[0] ).text();
  //console.log(s1text, s2text, s3text, s4text, s5text, s6text, s7text, s8text, s9text);
  let result = false;
  let winner;
  ["X", "O"].forEach(function(l) {
    if (
      (s1text === l && s2text === l && s3text === l) ||
      (s4text === l && s5text === l && s6text === l) ||
      (s7text === l && s8text === l && s9text === l) ||
      (s1text === l && s4text === l && s7text === l) ||
      (s2text === l && s5text === l && s8text === l) ||
      (s3text === l && s6text === l && s9text === l) ||
      (s1text === l && s5text === l && s9text === l) ||
      (s3text === l && s5text === l && s7text === l) )
    { result = true;
      winner = l;
    }
  })
  if (!!s1text && !!s2text && !!s3text && !!s4text && !!s5text && !!s6text && !!s7text && !!s8text && !!s9text) {
    result = true;
    winner = "tie";
  }
  if (result)
